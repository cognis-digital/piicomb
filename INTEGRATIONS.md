...
**piicomb** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `piicomb` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
piicomb ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
piicomb ... --format json | cognis-connect emit --to splunk --url $URL --token $TOK   # Splunk HEC
piicomb ... --format json | cognis-connect emit --to elastic --url $URL --token $TOK   # Elastic _bulk
piicomb ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `piicomb` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="piicomb") for rec in records]   # records = your JSON output
