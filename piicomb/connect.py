"""Native cognis-connect emit for piicomb — forward findings to any platform.

Maps piicomb's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    piicomb ... --format json | piicomb-emit --to stix
    piicomb-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        if 'pii' in rec:
            out['title'] = f"PII Found: {rec['pii']['type']}"
            out['severity'] = 'medium'
            out['type'] = rec['pii']['type']
            out['description'] = f"Found {rec['pii']['type']} in the text."
            out['tags'] = ['pii']
            if 'value' in rec['pii']:
                out['value'] = rec['pii']['value']
        if 'ipv4' in rec:
            out['ipv4'] = rec['ipv4']
        if 'domain' in rec:
